            ...
            <div class="row">
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        <label for="" class="form-label">Point of departure <span style="color: red">*</span></label>
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        ...
                        <input name="budget" type="number" placeholder="Enter Budget"  required onchange="enableDesField(),getEstimatedAmount()">
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="form-group">
                        <label for="" class="form-label">Start Date <span style="color: red">*</span></label>
                        <input  name="start_date"  type="date" placeholder="Start Date" required onchange="enableDesField(),getEstimatedAmount()">
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="form-group">
                        <label for="" class="form-label">End Date <span style="color: red">*</span></label>
                        <input name="end_date" type="date" placeholder="End Date"  required onchange="enableDesField(),getEstimatedAmount()">
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <input id="city-input" name="destination" type="text" placeholder="Enter a city" required disabled>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="form-group">
                        <label for="" class="form-label">Estimated Amount<span style="color: red">*</span></label>
                        <input name="estimated_amount" type="text" placeholder="Estimated" required value="0" disabled>
        ...
        $('#start-city-input').change(function () {
            ...
            success: function (response) {
                ...
                var radioButton = '<div class="card my-1">' +
                    ...
                    '<input class="form-check-input" type="radio" id="flexCheckDefault' + driverData.id + '" name="driver" value="' + driverData.id + '" data-price="'+driverData.drivers[0].price_per_day+'" onchange="getEstimatedAmount()">' +
                    '<label class="form-check-label" for="flexCheckDefault' + driverData.id + '">' +
                    driverData.name +
                    '</label>' +
                    '</div>' +
                    '<div>';
                ...
                var mainPropertiesToDisplay = ['name', 'email', 'city', 'state', 'phone', 'age'];

                // Add an inner loop for the nested driver details
                if (driverData.hasOwnProperty('drivers') && Array.isArray(driverData.drivers)) {
                    $.each(driverData.drivers, function (nestedIndex, nestedDriver) {
                        // Display specific properties from the nested driver object
                        var nestedPropertiesToDisplay = ['address', 'driver_personal_info', 'price_per_day', 'vehicle_type', 'availability', 'model', 'manufacturer'];

                        $.each(nestedPropertiesToDisplay, function (nestedPropIndex, nestedPropKey) {
                            var nestedLowercaseKey = nestedPropKey.toLowerCase();

                            if (nestedLowercaseKey in nestedDriver) {
                                radioButton += '<strong>' + nestedPropKey.replace(/_/g, ' ') + ':</strong> ' + nestedDriver[nestedLowercaseKey] + '<br>';
                            }
                        });
                    });
                }
                ...
                $.each(mainPropertiesToDisplay, function (mainIndex, mainKey) {
                    var mainLowercaseKey = mainKey.toLowerCase();

                    if (mainLowercaseKey in driverData) {
                        radioButton += '<strong>' + mainKey.replace(/_/g, ' ') + ':</strong> ' + driverData[mainLowercaseKey] + '<br>';
                    }
                });
...
function showHotels2(geoId){
    ...
    fetch("https://tripadvisor16.p.rapidapi.com/api/v1/hotels/searchHotels?geoId="+geoId+"&checkIn=2024-01-18&checkOut=2024-01-31&pageNumber=1&currencyCode=PKR", requestOptions)
    .then(response => response.text())
    .then(result => {
        ...
        data.forEach(place => {
            ...
            if(hotelPhoto){
                ...
                let updatedUrlUpdated = hotelPhoto.replace("{width}", "300").replace("{height}", "300");
...
function getEstimatedAmount(){
    ...
    setTimeout(() => {
        ...
        if(estimatedAmount>budgetAount||isNaN(budgetAount)||budgetAount==0){
            $('.estimated_amount_message').addClass('text-danger').removeClass('text-success').text('Budget is not enough for this trip');
            $('button[type="submit"]').attr('disabled',true);
        }else{
            $('.estimated_amount_message').addClass('text-success').removeClass('text-danger').text('All good');
