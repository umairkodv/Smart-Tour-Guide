...
<section class="inner-banner-wrap">
    <div class="inner-baner-container"
        style="background-image: url({{ asset('assets/images/inner-banner.jpg') }});">
        <div class="container">
                    ...
                    <div class="col-md-12 comment-sec">
                        ...
                        <form method="POST" action="{{ route('add.comment') }}">
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="comment" rows="4" cols="30" class="form-control" placeholder="Enter you comment about platform" required></textarea>
                            </div>
                                        ...
                                        @foreach ($values as $value)
                                            @if ($itinery->key == 'driver')
                                                ...
                                                @if ($driver)
                                                    ...
                                                    @endphp
                                                <a href="javascript:void(0)" class="show_driv" data-age={{ $driver_details->age }} data-price="{{ $driver_details->price_per_day }}" data-model="{{ $driver_details->model }}" data-ctype="{{ $driver_details->vehicle_type }}" data-manufacturer="{{ $driver_details->manufacturer }}" data-end="{{ $driver->id }}" data-name="{{ $driver->name }}" data-phone="{{ $driver->phone }}" data-bs-toggle="modal" data-bs-target="#driverDetailsModal">{{ $driver->name }}</a>
                                                @else
                                                    <span class="text-danger">User not found for ID: {{ $value }}</span>
                                                @endif
                                            @else
...
@push('scripts')
<script
    src="https://maps.googleapis.com/maps/api/js?key={{ config('app.google_map_api_key') }}&libraries=places"></script>
<script>
    var map;
...
document.addEventListener('click', function (event) {
    ...
    if (response.routes && response.routes.length > 0) {
        ...
        var mobileNavigationLink = '<a href="https://www.google.com/maps/dir/?api=1&travelmode=driving&dir_action=navigate&destination='
